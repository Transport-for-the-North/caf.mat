...
caf.tem import as ctem is currently not used but left in as the future idea is that it can all be called from here.
...
def run_prior_adjustment(params: PriorAdjustmentConf):
    ...
    if params.run_conversion:
        ...
        convert_ufms(params.post_me_dir, converter, params.post_me_out_dir, "postme_2023")

        convert_ufms(params.prior_dir, converter, params.prior_out_dir, "prior_2023")
    ...
    if params.run_mts_adj:
        mts_adj_factors(params.post_me_dir, params.prior_out_dir, params.main_out_dir)
...
def tripend_factors(
    ...
    hb_prod_fr_uc = (
        hb_prod_fr.aggregate_comp_zones(NORMITS)
        .translate_zoning(NOHAM_SECTOR, trans_vector=normits_noham_sector)
        .add_segments(["userclass"])
        .aggregate(["m", "userclass", "tp"])
    )
    hb_attr_fr_uc = (
        hb_attr_fr.aggregate_comp_zones(NORMITS)
        .translate_zoning(NOHAM_SECTOR, trans_vector=normits_noham_sector)
        .add_segments(["userclass"])
        .aggregate(["m", "userclass", "tp"])
    )
    hb_prod_to_uc = (
        hb_prod_to.aggregate_comp_zones(NORMITS)
        .translate_zoning(NOHAM_SECTOR, trans_vector=normits_noham_sector)
        .add_segments(["userclass"])
        .aggregate(["m", "userclass", "tp"])
    )
    hb_attr_to_uc = (
        hb_attr_to.aggregate_comp_zones(NORMITS)
        .translate_zoning(NOHAM_SECTOR, trans_vector=normits_noham_sector)
        .add_segments(["userclass"])
        .aggregate(["m", "userclass", "tp"])
    )
    nhb_prod_uc = (
        nhb_prod.aggregate_comp_zones(NORMITS)
        .translate_zoning(NOHAM_SECTOR, trans_vector=normits_noham_sector)
        .add_segments(["userclass"])
        .aggregate(["m", "userclass", "tp"])
    )
    nhb_attr_uc = (
        nhb_attr.aggregate_comp_zones(NORMITS)
        .translate_zoning(NOHAM_SECTOR, trans_vector=normits_noham_sector)
        .add_segments(["userclass"])
        .aggregate(["m", "userclass", "tp"])
    )
    ...
    prior_te_nhb = {}
    prior_te_fr["O"] = (
        synthetic_dvecs["O"].filter_segment_value("direction_od", 1) / hb_prod_fr_uc
    )
    prior_te_fr["D"] = (
        synthetic_dvecs["D"].filter_segment_value("direction_od", 1) / hb_attr_fr_uc
    )
    prior_te_to["O"] = (
        synthetic_dvecs["O"].filter_segment_value("direction_od", 2) / hb_prod_to_uc
    )
    prior_te_to["D"] = (
        synthetic_dvecs["D"].filter_segment_value("direction_od", 2) / hb_attr_to_uc
    )
    prior_te_nhb["O"] = (
        synthetic_dvecs["O"].filter_segment_value("direction_od", 0) / nhb_prod_uc
    )
    prior_te_nhb["D"] = (
        synthetic_dvecs["D"].filter_segment_value("direction_od", 0) / nhb_attr_uc
    )
    ...
    for orig in ["O", "D"]:
        ...
        factor_fr.add_segments(["p"]).remove_segment("userclass").save(
            out_dir / f"post_me_adj_fr_{orig}.dvec"
        )
        factor_to = prior_te_to[orig] * post_prior[orig]
        factor_to.add_segments(["p"]).remove_segment("userclass").save(
            out_dir / f"post_me_adj_to_{orig}.dvec"
        )
        factor_nhb = prior_te_nhb[orig] * post_prior[orig]
        factor_nhb.add_segments(["p"]).remove_segment("userclass").save(
            out_dir / f"post_me_adj_nhb_{orig}.dvec"
        )


# pylint: disable=too-many-locals
def mts_adj_factors(postme_dir: Path, synth_dir: Path, out_dir: Path):
    """
    Currently not implemented, but leaving here for future development
    """
    seg = cb.Segmentation(
        cb.SegmentationInput(
            enum_segments=["m", "userclass", "direction_od", "tp"],
            naming_order=["m", "tp", "userclass", "direction_od"],
            subsets={"m": [3], "tp": [1, 2, 3]},
        )
    )
    synth = MatrixFiles(segmentation_=seg, zoning=NOHAM, type_=MatrixType.OD, folder=synth_dir)

    postme = MatrixFiles(
        segmentation_=seg, zoning=NOHAM, type_=MatrixType.OD, folder=postme_dir
    )
    ...
    od_pa = {"O": "P", "D": "A"}
    for curr_dir in ["O", "D"]:
        synth_dvec = synth_dvecs[curr_dir].filter_segment_value(
            "direction_od", 1, keep_filtered=True
        )
        postme_dvec = post_dvecs[curr_dir].filter_segment_value(
            "direction_od", 1, keep_filtered=True
        )
        ...
        adj.save(out_dir / f"mts_adj_{od_pa[curr_dir]}")


if __name__ == "__main__":
    post_me_conf = OD2PAParameters.load_yaml(
        Path(r"C:\Users\YanZhu\caf_mat\postme_adj\post_me_conf.yml")
    )
    prior_conf = OD2PAParameters.load_yaml(
        Path(r"C:\Users\YanZhu\caf_mat\postme_adj\synthetic_conf.yml")
    )
