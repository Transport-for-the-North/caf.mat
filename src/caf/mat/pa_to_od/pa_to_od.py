...
def od_to_pa(
    ...
    fh_factors = tour_props.groupby(level=['from', 'o', 'd']).sum().squeeze().unstack(level='from')
    th_factors = tour_props.groupby(level=['to', 'o', 'd']).sum().squeeze().unstack(level='to')
    fh_check = fh_factors.mul(pa_24, axis=0)
    th_check = th_factors.mul(pa_24, axis=0)
...
if __name__ == "__main__":
    ...
    phi = phi.groupby(["mode", "purpose.fr"] + col_calc, observed=False)[["trips.est"]].sum().reset_index()
    phi = phi.rename(columns={"purpose.fr": "purpose", 'period.fr': 'tp_from', 'period': 'tp_to'})
    ...
    occ_factors = pd.read_csv(r"I:\NTS\outputs\occs\vehicle_occupancy_sum.csv", index_col=1).rename(p_uc).reset_index()
    occ_factors = occ_factors.groupby(['purpose', 'direction', 'period'])[['driver', 'total']].sum()
